  ...
  try {
    ...
    try {
		...
		for(unsigned int i=0; i < input.size(); i++)
		{
			...
			// when there is a first sequence, add this
			if (sum == NULL)
				sum = inputSeq;
			else // after, the second, third... has to be appended in the end
				sum->add_sequence(inputSeq);
		}
		
		std::fstream output(sequence.c_str(), 
							std::ios::out | std::ios::trunc | std::ios::binary);
		sum->save(output, "fc-cat", version->getVersion());// why the hell is this also stored?
		output.close();
