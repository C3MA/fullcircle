...
float PerlinNoise::get_2d(float x, float y)
{
  float vec[2];
  vec[0] = x;
  vec[1] = y;
  float noise= perlin_noise_2D(vec);
  // the perlin routines return a float. Normalize
  // this number to be in the interval [0,1].
  noise=fabs((noise+1.0)/2.0);
  if (noise > 1.0)
    noise = 1.0;
  return noise;
}
...
float PerlinNoise::get_3d(float x, float y, float z)
{
  ...
  vec[1] = y;
  vec[2] = z;
  float noise = perlin_noise_3D(vec);
  // the perlin routines return a float. Normalize
  // this number to be in the interval [0,1].
  noise=fabs((noise+1.0)/2.0);
  if (noise > 1.0)
...
/*
 * Implementation of Perlin's algorithm below - not developed by gonium.
 * See http://www.flipcode.com/archives/Perlin_Noise_Class.shtml
 */
...
float PerlinNoise::perlin_noise_3D(float vec[3]) {
  int terms    = _octaves;
  float freq   = _frequency;
  float result = 0.0f;
  float amp = _amplitude;
